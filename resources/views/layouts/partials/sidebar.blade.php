@php
    ...
    $menus = [
        'Utama' => [
            ['label' => 'Dashboard', 'route' => 'dashboard', 'can' => 'dashboard.lihat', 'icon' => 'M3 12l2-2m0 0l7-7 7 7M5 10v10a1 1 0 001 1h3m10-11l2 2m-2-2v10a1 1 0 01-1 1h-3m-6 0a1 1 0 001-1v-4a1 1 0 011-1h2a1 1 0 011 1v4a1 1 0 001 1m-6 0h6'],
        ],
        'Master Data' => [
...
<aside id="sidebar" class="fixed inset-y-0 left-0 z-40 flex w-64 -translate-x-full flex-col bg-slate-900 transition-transform duration-200 lg:translate-x-0">
    <div class="flex h-16 items-center gap-3 border-b border-slate-800 px-5">
        <div class="flex h-9 w-9 items-center justify-center rounded-lg bg-emerald-600 text-white">
            <svg class="h-5 w-5" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" d="M12 6v12m6-6H6"/></svg>
        </div>
        <div>
            ...
            @if (count($visible))
                ...
                @foreach ($visible as $item)
                    @php
                        ...
                        $prefix = str($item['route'])->beforeLast('.')->toString();
                        $active = $exists && (request()->routeIs($item['route']) || request()->routeIs($prefix.'.*'));
                    @endphp
